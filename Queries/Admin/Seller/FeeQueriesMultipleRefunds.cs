...
{
    public class FeeQueriesMultipleRefunds : StartBrowser
    {
        ...
        public string CreditCardUSPercentageFeeRefunded(string orderNumber, string lastSellerOrderFeeId)
        {
            //string sql = "Select sof.Amt from dbo.SellerOrderFee sof Inner Join dbo.SellerOrder so on sof.SellerOrderId = so.SellerOrderId Inner Join dbo.FeeType f on sof.FeeTypeId = f.FeeTypeId " +
            //"Where so.OrderNumber = '" + orderNumber + "' and sof.RateProcessingTypeId = 2 and f.Name = 'Credit Card US' and sof.FlatRate = '0.00'";
            ...
            "Where so.OrderNumber = '" + orderNumber + "' and sof.RateProcessingTypeId = 2 and f.Name = 'Credit Card US' and sof.FlatRate = '0.00' and SellerOrderFeeId > " + lastSellerOrderFeeId + " order by SellerOrderFeeId desc";
            string creditCardUSFee = DBSingleResultHelpers.DB_Method(sql);            
            if (creditCardUSFee == "")
            {
                creditCardUSFee = "0";
        ...
        public string CreditCardUSBaseFeeRefunded(string orderNumber, string lastSellerOrderFeeId)
        {
            //string sql = "Select sof.Amt from dbo.SellerOrderFee sof Inner Join dbo.SellerOrder so on sof.SellerOrderId = so.SellerOrderId Inner Join dbo.FeeType f on sof.FeeTypeId = f.FeeTypeId " +
            //"Where so.OrderNumber = '" + orderNumber + "' and sof.RateProcessingTypeId = 2 and f.Name = 'Credit Card US' and sof.PercentRate = '0.00'";
            string sql = "Select sof.Amt from dbo.SellerOrderFee sof Inner Join dbo.SellerOrder so on sof.SellerOrderId = so.SellerOrderId Inner Join dbo.FeeType f on sof.FeeTypeId = f.FeeTypeId " +
            "Where so.OrderNumber = '" + orderNumber + "' and sof.RateProcessingTypeId = 2 and f.Name = 'Credit Card US' and sof.PercentRate = '0.00' and SellerOrderFeeId > " + lastSellerOrderFeeId + " order by SellerOrderFeeId desc";
            string value = DBSingleResultHelpers.DB_Method(sql);
            return value;
        }

        //SellerOrderFee table: US Store Credit Fees for a refunded seller order.  (This is the percent rate. )
        public string StoreCreditUSPercentageFeeRefunded(string orderNumber, string lastSellerOrderFeeId)
        {
            //string sql = "Select sof.Amt from dbo.SellerOrderFee sof Inner Join dbo.SellerOrder so on sof.SellerOrderId = so.SellerOrderId Inner Join dbo.FeeType f on sof.FeeTypeId = f.FeeTypeId " +
            //"Where so.OrderNumber = '" + orderNumber + "' and sof.RateProcessingTypeId = 2 and f.Name = 'Store Credit US' and sof.FlatRate = '0.00'";
            string sql = "Select top 1 sof.Amt from dbo.SellerOrderFee sof Inner Join dbo.SellerOrder so on sof.SellerOrderId = so.SellerOrderId Inner Join dbo.FeeType f on sof.FeeTypeId = f.FeeTypeId " +
            "Where so.OrderNumber = '" + orderNumber + "' and sof.RateProcessingTypeId = 2 and f.Name = 'Store Credit US' and sof.FlatRate = '0.00' and SellerOrderFeeId > " + lastSellerOrderFeeId + " order by SellerOrderFeeId desc";
            string storeCreditUSFee = DBSingleResultHelpers.DB_Method(sql);
            if (storeCreditUSFee == "")
            {
                storeCreditUSFee = "0";
        ...
        //SellerOrderFee table: US Store Credit Fees for a refunded seller order.  (This is the base rate.  Only refunded for full refunds.)
        public string StoreCreditUSBaseFeeRefunded(string orderNumber, string lastSellerOrderFeeId)
        {
            //string sql = "Select sof.Amt from dbo.SellerOrderFee sof Inner Join dbo.SellerOrder so on sof.SellerOrderId = so.SellerOrderId Inner Join dbo.FeeType f on sof.FeeTypeId = f.FeeTypeId " +
            //"Where so.OrderNumber = '" + orderNumber + "' and sof.RateProcessingTypeId = 2 and f.Name = 'Store Credit US' and sof.PercentRate = '0.00'";
            string sql = "Select top 1 sof.Amt from dbo.SellerOrderFee sof Inner Join dbo.SellerOrder so on sof.SellerOrderId = so.SellerOrderId Inner Join dbo.FeeType f on sof.FeeTypeId = f.FeeTypeId " +
            "Where so.OrderNumber = '" + orderNumber + "' and sof.RateProcessingTypeId = 2 and f.Name = 'Store Credit US' and sof.PercentRate = '0.00' and SellerOrderFeeId > " + lastSellerOrderFeeId + " order by SellerOrderFeeId desc";
            string value = DBSingleResultHelpers.DB_Method(sql);
            return value;
        }

        // table: Commission Fee for a refunded seller order 
        public string CommissionFeeRefunded(string orderNumber, string lastSellerOrderFeeId)
        {
            //string sql = "Select sof.Amt from dbo.SellerOrderFee sof Inner Join dbo.SellerOrder so on sof.SellerOrderId = so.SellerOrderId Inner Join dbo.FeeType f on sof.FeeTypeId = f.FeeTypeId " +
            //"Where so.OrderNumber = '" + orderNumber + "' and sof.RateProcessingTypeId = 2 and f.Name = 'Commission'";
            string sql = "Select top 1 sof.Amt from dbo.SellerOrderFee sof Inner Join dbo.SellerOrder so on sof.SellerOrderId = so.SellerOrderId Inner Join dbo.FeeType f on sof.FeeTypeId = f.FeeTypeId " +
            "Where so.OrderNumber = '" + orderNumber + "' and sof.RateProcessingTypeId = 2 and f.Name = 'Commission' and SellerOrderFeeId > " + lastSellerOrderFeeId + " order by SellerOrderFeeId desc";
            string commissionFee = DBSingleResultHelpers.DB_Method(sql);
            //If Null, convert it to 0.
            if (commissionFee == "")
            {
        ...
        public string ShippingFeeRefunded(string orderNumber, string lastSellerOrderFeeId)
        {
            //string sql = "Select sof.Amt from dbo.SellerOrderFee sof Inner Join dbo.SellerOrder so on sof.SellerOrderId = so.SellerOrderId Inner Join dbo.FeeType f on sof.FeeTypeId = f.FeeTypeId " +
            //"Where so.OrderNumber = '" + orderNumber + "' and sof.RateProcessingTypeId = 2 and f.Name = 'Shipping Cost'";
            string sql = "Select top 1 sof.Amt from dbo.SellerOrderFee sof Inner Join dbo.SellerOrder so on sof.SellerOrderId = so.SellerOrderId Inner Join dbo.FeeType f on sof.FeeTypeId = f.FeeTypeId " +
            "Where so.OrderNumber = '" + orderNumber + "' and sof.RateProcessingTypeId = 2 and f.Name = 'Shipping Cost' and SellerOrderFeeId > " + lastSellerOrderFeeId + " order by SellerOrderFeeId desc";
            string shippingFee = DBSingleResultHelpers.DB_Method(sql);
            //If Null, convert it to 0.
            if (shippingFee == "")
            {
        ...
        //SellerOrderFee table: Shipping Cost for a refunded seller order         
        public string DirectProgramFeeRefunded(string orderNumber, string lastSellerOrderFeeId)
        {
            string sql = "Select top 1 sof.Amt from dbo.SellerOrderFee sof Inner Join dbo.SellerOrder so on sof.SellerOrderId = so.SellerOrderId Inner Join dbo.FeeType f on sof.FeeTypeId = f.FeeTypeId " +
            "Where so.OrderNumber = '" + orderNumber + "' and sof.RateProcessingTypeId = 2 and f.Name = 'Direct Progam Fee' and SellerOrderFeeId > " + lastSellerOrderFeeId + " order by SellerOrderFeeId desc";
            string shippingFee = DBSingleResultHelpers.DB_Method(sql);
            //If Null, convert it to 0.
            if (shippingFee == "")
            {
